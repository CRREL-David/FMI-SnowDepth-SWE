...
#Purpose is to aid in estimating snow depth and snow water equivalent (SWE) with models CNN, RF, SVM, ANN, LM, and an ensemble (EA) model over unknown areas from gathered field, airborne, and spaceborne data.
...
svm_Model <- train(Class  ~ .,
                   ...
                   tuneGrid = expand.grid(degree = c(2, 3), #Polynomial degrees.
                                          C = c(1.0, 10.0, 100.0), #Cost parameter.
...
nnet_Model <- train(Class  ~ .,
                    ...
                    trControl = control,
                    metric = "Rsquared",
                    tuneGrid = expand.grid(size = seq(from = 1, to = 2, by = 1), #Size of units in hidden layer.
...
Model_Weights <- R2_Num[1] + R2_Num[2] + R2_Num[3] + R2_Num[4] + R2_Num[5] #Generate the weights for each of the base models based off of the respective R2 value of each.
...
Pred_1[Pred_1 < 0] <- 0 #Replace any negative predicted values.
