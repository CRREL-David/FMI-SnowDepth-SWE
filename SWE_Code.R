...
#Purpose is to aid in estimating snow depth and snow water equivalent (SWE) with models CNN, RF, SVM, FFN, LM, and an ensemble (EA) model over unknown areas from gathered field, airborne, and spaceborne data.
...
set.seed(10)
#CNN
x_0 <- as.matrix(samples[,-102])  #Predictors.
y_0 <- as.matrix(samples[,102])   #Target variable.
x_rows <- as.matrix(nrow(x_0))    #Number of rows.

x_Data <- as.matrix(Data[,-102])  #Predictors for Data.

# Standardize data and perform PCA
original_pca <- prcomp(x_0, center = TRUE, scale. = TRUE)
# Get the proportion of variance explained
explained_variance <- summary(original_pca)$importance[2, ]
# Calculate cumulative variance
cumulative_variance <- cumsum(explained_variance)
# Find the number of PCs that explain at least 95% variance
num_pcs <- which(cumulative_variance >= 0.95)[1]
# Export the significant PCs
x <- original_pca$x[, 1:num_pcs]

# Project new data onto the same PCA space
new_data_pcs <- predict(original_pca, newdata = x_Data)
# Select the same significant PCs
x_FinalData <- new_data_pcs[, 1:num_pcs]
...
plot(x_axes, y_0, ylim = c(min(ypred), max(y_0)),
     col = "burlywood", type = "l", lwd = 2, xlab = "Predicted SWE", ylab = "Field SWE")
...
y_mean1 <- mean(cnn_res1$Observed)
SST1 <- sum((cnn_res1$Observed - y_mean1)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred1 <- predict(cnn_LR)
SSR1 <- sum((y_pred1 - y_mean1)^2)

# Calculate R-squared
cnn_rSquared <- format(signif(SSR1 / SST1, digits = 2), nsmall = 2) #Model Rsquared.
...
# Calculate SST (Total Sum of Squares)
y_mean2 <- mean(rf_res1$Observed)
SST2 <- sum((rf_res1$Observed - y_mean2)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred2 <- predict(rf_LR)
SSR2 <- sum((y_pred2 - y_mean2)^2)

# Calculate R-squared
rf_rSquared <- format(signif(SSR2 / SST2, digits = 2), nsmall = 2) #Model Rsquared.
...
# Calculate SST (Total Sum of Squares)
y_mean3 <- mean(svm_res1$Observed)
SST3 <- sum((svm_res1$Observed - y_mean3)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred3 <- predict(svm_LR)
SSR3 <- sum((y_pred3 - y_mean3)^2)

# Calculate R-squared
svm_rSquared <- format(signif(SSR3 / SST3, digits = 2), nsmall = 2) #Model Rsquared.
...
nnet_Model <- train(Class  ~ .,
                    ...
                    trControl = control,
                    metric = "Rsquared",
                    tuneGrid = expand.grid(size = seq(from = 2, to = 4, by = 1), #Size of units in hidden layer.
...
# Calculate SST (Total Sum of Squares)
y_mean4 <- mean(nnet_res1$Observed)
SST4 <- sum((nnet_res1$Observed - y_mean4)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred4 <- predict(nnet_LR)
SSR4 <- sum((y_pred4 - y_mean4)^2)

# Calculate R-squared
nnet_rSquared <- format(signif(SSR4 / SST4, digits = 2), nsmall = 2) #Model Rsquared.
...
# Calculate SST (Total Sum of Squares)
y_mean5 <- mean(lm_res1$Observed)
SST5 <- sum((lm_res1$Observed - y_mean5)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred5 <- predict(lm_LR)
SSR5 <- sum((y_pred5 - y_mean5)^2)

# Calculate R-squared
lm_rSquared <- format(signif(SSR5 / SST5, digits = 2), nsmall = 2) #Model Rsquared.
...
set.seed(42)
## EA ##
#Combined model outputs from CNN, RF, SVM, FFN, and MLR are used to determine the ensemble (EA) outcome based on their R2 value.
...
# Calculate SST (Total Sum of Squares)
y_mean6 <- mean(ea_res2$Observed)
SST6 <- sum((ea_res2$Observed - y_mean6)^2)
# Calculate SSR (Sum of Squares for Regression)
y_pred6 <- predict(ea_LR)
SSR6 <- sum((y_pred6 - y_mean6)^2)

# Calculate R-squared
ea_rSquared <- format(signif(SSR6 / SST6, digits = 2), nsmall = 2) #Model Rsquared.
...
Pred_1 <- data.frame(data.frame(model %>% predict(x_FinalData)), #Runs the CNN model on the image objects.
                     ...
                     data.frame(predict(nnet_Model, Data)), #Runs the FFN model on the image objects.
...
write.csv(Model_Stats_transpose, ".csv", row.names = FALSE) #Export final model metrics from all models.